...
public static class GlobalPool<T> where T : class, new()
{
	...
	class TypeConstrainedPool<TT> : DynamicPool<TT> where TT : class, new()
	{
	...
	static readonly TypeConstrainedPool<T> pool = new TypeConstrainedPool<T>(() => new T());
