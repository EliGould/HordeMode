...
public abstract class AppBase : MonoBehaviour
{
	...
	protected virtual void AtGUI() { }

	protected virtual void AtTimeScaleChanged(float newTimeScale) { }
...
public sealed partial class App : AppBase
{
	...
	{
		get { return instance._timeScale.Current; }
	}

	public static void SetTimeScale(float? timeScale, object setter)
	{
		if(timeScale == null) { instance._timeScale.Remove(setter); }
		else
		{
			float newScale = Mathf.Clamp(timeScale.Value, -100.0f, 100.0f);
			instance._timeScale.Set(newScale, setter);
		}
	}

	public static bool isFixedFrame
	{
	...
	protected void Update()
	{
		...
		if(currentAppState != null && currentAppState)
		{
	...
	{
		AtTimeScaleChanged(newTimeScale);
	}
	#endregion // Lifecycle

	#region State Lifecycle
	AppState FindAndInitializeAppState()
	{
		// Start off just picking a sibling component
		// if present
		AppState appState = GetComponent<AppState>();
		...
		Dbg.Log(this, "APP: Starting with state {0}", appState);

		if(appState.transform != transform)
		{
			appState.transform.parent = transform;
		}

		PreinitializeState(appState);
	...
	GameObject InstantiateAndChild(GameObject prefab)
	{
